...
This represents the probability of occupation (in the four cardinal directions) of landscape mosaics at a given distance from the closer current species occurrence. It is the dispersal model (DM) produced by MetaLandSim.
