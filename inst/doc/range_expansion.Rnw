...
The previous range expansion simulation produces a graphic output (which opens in a html page):
...
This represents the probability of occupation (in the four cardinal directions) of landscape mosaics at a given distance from the closer current species occurrence.
...
This application of the expansion model to the current species occurrences (B) provides a future occurrence model based only on dispersal ability and landscape configuration, such as:
...
The four sub-models (A), considering each of the cardinal directions, are combined in a final dispersal model (C).
