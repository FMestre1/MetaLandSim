...
\hlkwd{data}\hldef{(occ.landscape)}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning in data(occ.landscape): data set 'occ.landscape' not found}}\begin{alltt}
\hlkwd{data}\hldef{(occ.landscape2)}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning in data(occ.landscape2): data set 'occ.landscape2' not found}}\begin{alltt}
\hlcom{#Using data with only one snapshot of the occupancy status}
\hldef{param1} \hlkwb{<-} \hlkwd{parameter.estimate} \hldef{(occ.landscape,} \hlkwc{method}\hldef{=}\hlsng{'Rsnap_1'}\hldef{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in parameter.estimate(occ.landscape, method = "{}Rsnap\_1"{}): could not find function "{}parameter.estimate"{}}}\begin{alltt}
\hldef{param1}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'param1' not found}}\begin{alltt}
\hlcom{#Using data with more than one snapshot of the occupancy status}
\hldef{param2} \hlkwb{<-} \hlkwd{parameter.estimate} \hldef{(occ.landscape2,} \hlkwc{method}\hldef{=}\hlsng{'Rsnap_x'}\hldef{,}
\hlkwc{nsnap}\hldef{=}\hlnum{10}\hldef{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in parameter.estimate(occ.landscape2, method = "{}Rsnap\_x"{}, nsnap = 10): could not find function "{}parameter.estimate"{}}}\begin{alltt}
\hldef{param2}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: object 'param2' not found}}\end{kframe}
\end{knitrout}
...
\hlcom{#First, generate the files to be the input of the application}
\hlkwd{parameter.estimate} \hldef{(occ.landscape2,} \hlkwc{method}\hldef{=}\hlsng{'MCsim'}\hldef{)}

\hlcom{#run the application mcm.exe from Moilanen (1999). }
\hlcom{#Previously read the readme.txt file #available with the }
\hlcom{#application. }
\hlcom{#Consider particularly the three step procedure for estimation, }
\hlcom{#using nonlinear regression (Hanski, 1994) to produce priors }
\hlcom{#for the Monte Carlo simulation). In the command line (first put }
\hlcom{#the application and the files in a folder with no spaces }
\hlcom{#in the name. }
\hlcom{#e.g.: 'C:/moilanen/'):}

\hlcom{#mce.exe inputMCsim.dat inputMCsim}

\hlcom{#Or, from R:}

\hlkwd{system}\hldef{(}\hlsng{'mce.exe inputMCsim.dat inputMCsim'}\hldef{)}


\hlcom{#After, create a data frame, with create.parameter.df, }
\hlcom{#using the estimated parameters:}
\hldef{param3} \hlkwb{<-} \hlkwd{create.parameter.df}\hldef{(alpha, x, y, e)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlcom{#Method 'rescue'}
\hlkwd{parameter.estimate} \hldef{(occ.landscape2,} \hlkwc{method}\hldef{=}\hlsng{'rescue'}\hldef{)}

\hlcom{#run the application file fmetapop_rescue.exe from }
\hlcom{#the command line (first put the application and the }
\hlcom{#files in a folder with no spaces in the name. }
\hlcom{#e.g.: 'C:/terbraak/'):}

\hlcom{#fmetapop_rescue input_rescue}

\hlcom{#Or, from R:}

\hlkwd{system}\hldef{(}\hlsng{'fmetapop_rescue input_rescue'}\hldef{)}

\hlcom{#After, create a data frame, with create.parameter.df, }
\hlcom{#using the estimated parameters:}
\hlcom{#param4 <- create.parameter.df(alpha, x, y, e)}

\hlcom{#Method 'norescue'}
\hlkwd{parameter.estimate} \hldef{(occ.landscape2,} \hlkwc{method}\hldef{=}\hlsng{'norescue'}\hldef{)}

\hlcom{#run the application file fmetapop_norescue.exe from }
\hlcom{#the command line (first put the application and the }
\hlcom{#files in a folder with no spaces in the name. }
\hlcom{#e.g.: 'C:/terbraak/'):}

\hlcom{#fmetapop_norescue input_norescue}

\hlcom{#Or, from R:}

\hlkwd{system}\hldef{(}\hlsng{'fmetapop_norescue input_norescue'}\hldef{)}

\hlcom{#After, create a data frame, with create.parameter.df, }
\hlcom{#using the estimated parameters:}

\hldef{param5} \hlkwb{<-} \hlkwd{create.parameter.df}\hldef{(alpha, x, y, e)}
\end{alltt}
\end{kframe}
\end{knitrout}
